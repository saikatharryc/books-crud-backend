...
module.exports = async function (fastify, opts, next) {
  fastify.route({
    ...
    handler: async function (request, reply) {
      try {
        const doc = await fastify.sql.models.Users.findOne({ $or: [{ email: request.body.username }, { username: request.body.username }] })
        let loggedIn = false
        // console.log(doc,request.body)
        if (doc) { // check if the account exists
          const compareRes = await fastify.comparePasswordHash(request.body.password, doc.password)
          if (compareRes === true) { loggedIn = true }
        }

        if (loggedIn === true) {
          const fakeip = (Math.floor(Math.random() * 255) + 1) + '.' + (Math.floor(Math.random() * 255) + 0) + '.' + (Math.floor(Math.random() * 255) + 0) + '.' + (Math.floor(Math.random() * 255) + 0)
          const ref = crypto.randomBytes(2).toString('hex')
          const sessionId = await fastify.setSessionId({ userid: doc.email, ipAddress: request.ip, ref: ref })
  ...
  fastify.route({
    ...
    handler: async function (request, reply) {
      try {
        const doc = await fastify.sql.models.Users.findOne({ $or: [{ email: request.body.email }, { username: request.body.username }] })
        console.log(doc)
        if (doc) { // check uf the account exists
          ...
          // hash the password
          const passwordHash = await fastify.generatePasswordHash(request.body.password)
