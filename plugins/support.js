...
module.exports = fp(function(fastify, opts, next) {
  ...
  fastify.decorate("createGenere", (name, description) => {
    return fastify.sql.models.Genere.create({
      name: name,
  ...
  fastify.decorate("listBooks", id => {
    if (id) {
      return fastify.sql.models.Book.findOne({
        where: { id: id },
        include: [
          {
            model: fastify.sql.models.bookAuthor,
            include: [fastify.sql.models.Author]
          },
          {
            model: fastify.sql.models.bookGenere,
            include: [fastify.sql.models.Genere]
          }
        ]
      });
    } else {
  ...
  fastify.decorate("removeGenereFromBook", (genere_id, book_id) => {
    return fastify.sql.models.bookGenere.destroy({
      where: { genere_id: genere_id, book_id: book_id }
    });
  });
  ...
  fastify.decorate("removeAuthorFromBook", (author_id, book_id) => {
    return fastify.sql.models.bookAuthor.destroy({
      where: { author_id: author_id, book_id: book_id }
    });
  });
  ...
  fastify.decorate("editBook", (updated_data, book_id) => {
    return fastify.sql.models.Book.update(
      { ...updated_data },
      { where: { id: book_id } }
    );
  });
